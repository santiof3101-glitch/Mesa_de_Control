...
async function guardarModulosSupabase() {
  ...
  for (const [modulo, datos] of Object.entries(snapshots)) {
    await guardarRegistroSupabase(modulo, "base", { ...datos, updatedAt: new Date().toISOString() }, usuario);
  }
}
...
async function restoreModulesFromSupabaseIfNeeded() {
  ...
  try {
    const [usuarios, catalogos, saneamientos, compras, contratos, proveedores, archivos] = await Promise.all([
      leerUltimoModuloSupabase("usuarios"),
      leerUltimoModuloSupabase("catalogos"),
      leerUltimoModuloSupabase("saneamientos"),
      leerUltimoModuloSupabase("compras"),
      leerUltimoModuloSupabase("contratos"),
      leerUltimoModuloSupabase("proveedores"),
      leerUltimoModuloSupabase("archivos")
    ]);
    const anyRemoteModule = [usuarios, catalogos, saneamientos, compras, contratos, proveedores, archivos].some(Boolean);
    let changed = false;
...
function touchSession() {
  if (session.role !== "public") lastActivityAt = Date.now();
}

function checkSessionExpiry() {
  if (session.role === "public") return;
  if (Date.now() - lastActivityAt <= SESSION_TIMEOUT_MS) return;
  setSession({ role: "public", userId: null, name: "", agency: "" });
  setView("acceso");
...
function createTask(data) {
  ...
  if (duplicateWarnings.length) {
    ...
    showToast("Solicitud enviada a control legal.");
  }
}
...
function renderTasks() {
  ...
  document.querySelector("#unassignedTaskCount").textContent = state.tasks.filter((task) => !task.legalUserId && task.status === "pendiente").length;
    ...
    card.innerHTML = `
      <div>
        ...
        <p class="task-meta">
          ...
          <span>${escapeHtml(task.ciudad)}</span>
...
function loginLegal(data) {
  const user = state.legalUsers.find((item) =>
    item.username.toLowerCase() === data.username.trim().toLowerCase() && item.password === data.password
  );
...
function getKpis(tasks = state.tasks) {
  ...
  const inProgress = tasks.filter((task) => task.status !== "pendiente" && !isClosedStatus(task.status)).length;
  const completed = tasks.filter((task) => isClosedStatus(task.status)).length;
  const unassigned = tasks.filter((task) => !task.legalUserId && task.status === "pendiente").length;
...
function renderStatusLookup() {
  ...
  statusResults.innerHTML = results.map((task) => `
    <article class="status-result-card">
      <div>
        <strong>${escapeHtml(task.cliente)}</strong>
        <span>${escapeHtml(task.placa)} | Cedula ${escapeHtml(task.cedula)}</span>
        <span>Asesor comercial: ${escapeHtml(task.asesor || "Sin registro")}</span>
      </div>
      <div>
        ${renderStatusPill(task.status)}
